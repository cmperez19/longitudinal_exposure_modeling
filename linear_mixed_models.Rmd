...
longitudinal_pfas = PFAS_24mo_6mo_60mo_placenta %>%
        ...
        select(contains(pfas), bf_length_mo, Sample_Name,  mom_age_at_birth,     official_enroll_category, gest_age_in_weeks_edd, bf_group,
...
summary(model_pfhxs)
confint(profile(model_pfhxs), level = 0.95)
```
...
summary(model_pfna)
confint(profile(model_pfna), level = 0.95)
```
...
summary(model_pfoa)
confint(profile(model_pfoa), level = 0.95)
```
...
summary(model_pfos)
confint(profile(model_pfos), level = 0.95)
```
#linear mixed methods with year * bf_duration
```{r, warning=FALSE}
for( i in 1:length(longitudinal_pfas)){
model <- lmer(concentration ~ bf_length_mo:year + (1 + year| Sample_Name) + mom_age_at_birth + official_enroll_category + gest_age_in_weeks_edd + mom_education + child_sex,  data=longitudinal_pfas[[i]])

print(pfas[i])
print(summary(model)) 
print(confint(profile(model), level = 0.95))
...
#repeated measures anova using long data
Error(subject/time): Specifies that subject is a random effect and time is nested within subject
https://www.geeksforgeeks.org/r-language/repeated-measures-within-subjects-anova-in-r/

```{r}
aov_model_pfhxs <- aov(concentration ~ bf_length_mo + Error(Sample_Name/year) + mom_age_at_birth + official_enroll_category + gest_age_in_weeks_edd + mom_education + child_sex , data = longitudinal_pfas$PFHxS_Total)
# View the summary of the ANOVA
summary(aov_model_pfhxs)

aov_model_pfoa <- aov(concentration ~ bf_length_mo + Error(Sample_Name/year) + mom_age_at_birth + official_enroll_category + gest_age_in_weeks_edd + mom_education + child_sex , data = longitudinal_pfas$PFOA)
# View the summary of the ANOVA
summary(aov_model_pfoa)

aov_model_pfos <- aov(concentration ~ bf_length_mo + Error(Sample_Name/year) + mom_age_at_birth + official_enroll_category + gest_age_in_weeks_edd + mom_education + child_sex , data = longitudinal_pfas$PFOS_Total)
# View the summary of the ANOVA
summary(aov_model_pfos)

aov_model_pfna <- aov(concentration ~ bf_length_mo + Error(Sample_Name/year) + mom_age_at_birth + official_enroll_category + gest_age_in_weeks_edd + mom_education + child_sex , data = longitudinal_pfas$PFNA)
# View the summary of the ANOVA
summary(aov_model_pfna)
```

```{r}
for( i in 1:4){
plot =  ggplot(aes( x = concentration, fill = factor(year)), data = longitudinal_pfas[[i]]) + geom_density(alpha = 0.5) + labs( title = names(longitudinal_pfas)[i])
print( plot)
}
```
...
for( i in 1:8){
p = ggplot(aes_string( x = pfas[i], fill = "bf_group"), data = PFAS_24mo_6mo_60mo_placenta) + geom_density(alpha = 0.5)
print(p)
}
```
...
```{r}
for( i in 1:4){
plot =  ggplot(aes( y = concentration, x = year, color = factor(bf_group)), data = longitudinal_pfas[[i]]) + geom_path() + labs( title = names(longitudinal_pfas)[i])
print( plot)
}
```
...
```{r}
knots = c(0.5,5)
linear_model <- lm( concentration ~ bf_group *  bs(year, knots = knots, degree = 1)+ mom_age_at_birth + official_enroll_category + gest_age_in_weeks_edd + mom_education + child_sex, data = longitudinal_pfas$PFOS_Total)
